	...
	public Transaction[] handleTxs(Transaction[] possibleTxs) 
	{
		...
		else
		{
			for(int x=0;x<possibleTxs.length;x++)
			{
				...
				{
					for(int i=0;i<possibleTxs[x].numInputs();i++)
					{
					        publicLedger.removeUTXO(new UTXO(possibleTxs[x].getInput(i).prevTxHash,possibleTxs[x].getInput(i).outputIndex));
					}

					for(int i=0;i<possibleTxs[x].numOutputs();i++)
					{
					        publicLedger.addUTXO(new UTXO(possibleTxs[x].getHash(),i),possibleTxs[x].getOutput(i));
					}
					...
					{				
						...
						if(testArray!=null)
							finalList.addAll(Arrays.asList(testArray));
					}
					...
					for(int i=0;i<possibleTxs[x].numOutputs();i++)
					{
					        publicLedger.removeUTXO(new UTXO(possibleTxs[x].getHash(),i));
					}
					

					for(int i=0;i<possibleTxs[x].numInputs();i++)
					{
					        publicLedger.addUTXO(new UTXO(possibleTxs[x].getInput(i).prevTxHash,possibleTxs[x].getInput(i).outputIndex),possibleTxs[x].getOutput(possibleTxs[x].getInput(i).outputIndex));
					}
